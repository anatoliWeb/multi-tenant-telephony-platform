...
Note: Hold/resume is currently a local/demo placeholder that only updates browser-side state. Transfer is code-level implemented in the Angular softphone with guarded SIP.js REFER support, extension-or-SIP-URI target normalization, and local fallback messaging when REFER is unavailable; live FreeSWITCH REFER verification is still pending. The DTMF keypad is complete, while the browser transport still depends on RTCDTMFSender support and SIP INFO fallback behavior per browser/session. Device selection is complete for pre-call setup and discovery, but live microphone switching during an active call remains disabled. Reconnect behavior now retries registration with bounded backoff on unexpected SIP transport drops, exposes reconnecting / failed transport state in the Angular softphone, and keeps manual retry available after retry exhaustion. Floating minimized mode now collapses the modal into a floating bubble without ending the call, and the bubble preserves call, mute, hold, and reconnect state until the user restores or closes it. Active calls are not preserved across transport loss unless the browser and PBX stack explicitly support that recovery path.

Stage 15 summary: the Angular call-control slice now covers registration, incoming/outgoing calls, accept/decline, hang up, mute/unmute, hold/resume placeholder, DTMF keypad with SIP INFO fallback, pre-call microphone selection, reconnect retry/backoff, minimized floating mode, and guarded transfer UI. Browser verification remains partial because remote audio and live REFER still need manual end-to-end confirmation in a real browser session.
