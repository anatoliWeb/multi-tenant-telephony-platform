...
class MetaServiceCacheSectionTest extends TestCase
{
    ...
    public function test_corrupt_roles_cache_rebuilds_roles_section_only(): void
    {
        ...
        Cache::put($rolesKey, collect([collect(['broken'])]), now()->addMinutes(10));
        $sentinelPermissions = new Collection([Permission::query()->first()]);
        ...
        $this->assertSame('admin', $meta['roles']->first()->name);
        $this->assertSame('users.view', $meta['permissions']->first()->name);
        $this->assertSame($sentinelPermissions->first()->id, Cache::get($permissionsKey)->first()->id);
    }
    ...
    public function test_corrupt_permissions_cache_rebuilds_permissions_section_only(): void
    {
        ...
        $sentinelRoles = new Collection([Role::query()->first()]);
        Cache::put($rolesKey, $sentinelRoles, now()->addMinutes(10));
        Cache::put($permissionsKey, collect([collect(['broken'])]), now()->addMinutes(10));
        ...
        $this->assertSame('users.view', $meta['permissions']->first()->name);
        $this->assertSame('admin', $meta['roles']->first()->name);
        $this->assertSame($sentinelRoles->first()->id, Cache::get($rolesKey)->first()->id);
    }
}
