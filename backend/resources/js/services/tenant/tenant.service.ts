...
export const tenantService = {
  listTenants: async (): Promise<TenantMembershipListPayload> => {
    ...
    return (response as ApiResponse<TenantMembershipListPayload>).data ?? { tenants: [], current_tenant_id: null };
  },
  currentTenant: async (): Promise<TenantContextPayload> => {
    const response = await api.get<TenantContextPayload>('/v1/user/tenant');
    return (response as ApiResponse<TenantContextPayload>).data ?? { tenant: null, membership: null, current_tenant_id: null };
  },
