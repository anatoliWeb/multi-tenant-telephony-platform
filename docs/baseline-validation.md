...
- The provisioning flow now rewrites the live `public.xml` and `default.xml` dialplans with a single inline local-demo block so local `1001 <-> 1002` and `2001 <-> 2002` browser calls can bridge through the runtime realm, and the local-demo block appears before the stock `Local_Extension` rules so the live Sofia contact bridge wins before any `bridge(user/...)` fallback.
